...
const activeGames = computed(
    () =>
        data.value?.filter((g: Game) => g.status === "in_progress").length ?? 0,
);
const waitingGames = computed(
    () => data.value?.filter((g: Game) => g.status === "waiting").length ?? 0,
);
const totalPlayers = computed(
    () =>
        data.value?.reduce((sum, game: Game) => sum + game.players.length, 0) ??
        0,
);
...
const filteredGames = computed(() => {
    ...
    return data.value.filter((game: Game) => {
        const matchesSearch = game.roomId
            .toLowerCase()
    ...
    <div class="min-h-screen w-full bg-background p-6 lg:p-8">
        ...
        <Card class="mb-6">
            ...
            <CardContent>
                <!-- Auto-refresh toggle -->
                <div
                    class="flex items-center justify-between mb-4 p-3 rounded-lg bg-muted/50"
                >
