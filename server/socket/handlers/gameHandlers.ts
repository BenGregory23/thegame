...
import { loggerB } from "~~/server/utils/consola-backtend";
export const gameHandler = (io: Server) => {
  const joinRoom = function (this: Socket, payload: IPayload) {
    loggerB.info("requesting to join room : ", payload.roomID);
    const socket = this;
  ...
  const leaveRoom = function (
    this: Socket,
    payload: { reason: DisconnectReason; description?: any },
  ) {
    const socket = this;
    const game = gameManager.getGame(socket.data.roomID);
    if (game) {
      loggerB.info(
        game.roomId + " - removing player from game " + socket.data.username,
      );
      game.removePlayer(socket.id);

      if (game.players.size === 0) {
        loggerB.warn(
          game.roomId + " - No players left in game {} | deleting room",
        );
  ...
  const startGame = function (this: Socket, payload: IPayload) {
    ...
    if (socket.id == game?.hostId) {
      game?.startGame();
      socket.to(payload.roomID).emit(Events.GAME_START, {
        roomId: payload.roomID,
        content: game.getPublicState(),
      });
      for (let player of game.players.values()) {
        io.to(player.id).emit(Events.PLAYER_STATE, {
          roomId: payload.roomID,
          content: game.getPlayerState(player.id),
        });
      }
    }
  };
  ...
  const playCard = function (this: Socket, payload: IPayload) {
    ...
    const isPlaced = game.playCard(
      socket.id,
      payload.content.card,
      payload.content.stackId,
    );
    if (isPlaced) {
  ...
  function updateState(socket: Socket, game: Game) {
    if (game.status === GameStatus.FINISHED) {
      ...
      io.to(game.roomId).emit(Events.GAME_LOSE, {
        ...
        content: {
          remainingCards:
            game.deck.length +
            Array.from(game.players.values()).reduce(
              (acc, curr) => acc + (curr.handSize ?? 0),
              0,
            ),
        },
      });
    ...
    socket.emit(Events.PLAYER_STATE, {
      roomId: game.roomId,
      content: game.getPlayerState(socket.id),
    });
    socket.to(game.roomId).emit(Events.GAME_STATE, {
      roomId: game.roomId,
      content: game.getPublicState(),
    });
  }
