...
export class Game {
  ...
  addPlayer(socketId: string, username: string) {
    ...
    this.players.set(socketId, {
      ...
      id: socketId,
      hand: [],
      isHost: socketId == this.hostId ? true : false,
    });
  }
  ...
  playCard(socketId: string, card: ICard, stackId: string): boolean {
    ...
    if (isPlacable) {
      ...
      if (
        this.currentNumberOfCardsPlaced <
          this.MIN_NUMBER_OF_CARDS_TO_PLACE_WITH_DECK &&
        !this.canPlayerPlaceAnyCard()
      ) {
        loggerB.warn(
          this.roomId + " - current player cannot place any card - game lost",
        );
  ...
  private replenishPlayerHand(socketId: string) {
    ...
    const numberOfCardsToDraw =
      this.MINIMUM_HAND_LENGTH - this.players.get(socketId)!.hand!.length;
  ...
  private canCardBePlacedOnStack(card: ICard, stack: IStack): boolean {
    ...
    switch (stack.type) {
      case StackType.INCREASE: {
        return (
          card.value > lastCard.value || card.value === lastCard.value - 10
        );
      }
